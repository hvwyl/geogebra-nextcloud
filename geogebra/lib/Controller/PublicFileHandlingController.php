...
namespace OCA\Geogebra\Controller;
...
class PublicFileHandlingController extends Controller{
    ...
    public function load($token) {
        ...
        if ($node instanceof Folder) {
            $fullpath = $this->request->getParam('path');
            if (!$fullpath) {
    ...
    public function save($token, $filecontents, $path, $mtime) {
        ...
        if($file->isUpdateable()) {
            if ($mtime != $file->getMTime()) {
                $this->logger->error("Anonymous cannot save shared ggb (someone updated it in the meantime): {$mtime} vs. {$file->getMTime()} {$file->getPath()}", ['app' => 'geogebra']);
            ...
            try {
                $file->putContent(base64_decode($filecontents));
            ...
            // Not writeable!
            $this->logger->error('User does not have permission to write to share file: ' . $file->getPath(), ['app' => 'geogebra']);
